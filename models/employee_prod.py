...
def employee_speed(db: str):
    ...
    df_orders.drop(['Reporting No','Offline No', 'Created By'], axis=1, inplace=True)
    ...
    if df_orders_hist.isnull().values.any():

        return "The Order History file has not been updated"
        ...
        result1.sort_values('Item/Second', ignore_index=True)

        results = result1.to_dict(orient="records")
