...
def train_models_task(db: str, yyyy: str, mm: str, dd: str):
    ...
    linreg = save_model_to_db(db, cy, yyyy, mm, dd)
    responses.append('Linear Regression has been updated')
    client = pymongo.MongoClient(
        'mongodb+srv://DataLog:DataLog@cluster0.jzr1zc7.mongodb.net/')
    client[db]['training_log'].update_one(
        {'flag': 'training_log'}, {"$set": {'isDone': True}})
    client[db]['training_log'].update_one(
        {'flag': 'training_log'}, {"$set": {'results': responses}})
    client[db]['training_log'].update_one({'flag': 'training_log'}, {
                                          "$set": {'latest_date_in_model': yyyy+'-'+mm+'-'+dd}})
    return responses
