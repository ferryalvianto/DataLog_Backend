...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    user_found = next(
        (u for u in users if u['username'] == form_data.username), {})
...
async def put_model_cat(db: str, category: str):
    response = load_saved_model_from_db_with_category(
        db, get_weather(), category)
    if response:
        return response
    raise HTTPException(400, f"Something went wrong")
...
async def upload_log(db: str, yyyy: str, mm: str, dd: str):
    ...
    col = mydb['df_sales']
    inptstr = yyyy+'-'+mm+'-'+dd
    results = col.find({"Date": inptstr, '$or': [{"Establishment": 0}, {
                       "Establishment": 1}]}, {"_id": 0, "Date": 1, "Establishment": 1})
    ...
    trained = client[db]['timeseries_models'].find(
        {}, {"_id": 0, "latest_date_in_model": 1})
    ...
    if (latestdate + timedelta(days=1)) == inpt or latestdate == inpt:
        if 'Establishment' and 'Date' in results.columns:
            ...
            res = col.find({"Date": latestdatestr, '$or': [{"Establishment": 0}, {
                           "Establishment": 1}]}, {"_id": 0, "Date": 1, "Establishment": 1})
...
def update_training_log(db: str):
    client = pymongo.MongoClient(
        'mongodb+srv://DataLog:DataLog@cluster0.jzr1zc7.mongodb.net/')
    collection = client[db]['training_log']
    collection.update_one({'flag': 'training_log'}, {
                          "$set": {'isDone': False}})
    return '1'
